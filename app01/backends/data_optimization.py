...
class DataStore(object):
    ...
    def process_save(self):
        ...
        :return:
        '''
        print "\033[42;1m -----process service data-----\033[0m"
        if self.data['status'] ==0:
            for key,data_val in settings.STATUS_DATA_OPTIMIZATION.items():
                data_key_in_redis = "StatusData_%s_%s_%s" %(self.client_id,self.service_name,key)
                print '\033[31;1m%s\033[0m'%data_key_in_redis
                last_point_from_redis = self.redis_obj.lrange(data_key_in_redis,-1,-1)
                if not last_point_from_redis:    #key不存在
                ...
                if data_val[0] == 0:
                    ...
                    last_point_data,last_point_save_time = json.loads(self.redis_obj.lrange(data_key_in_redis,-1,-1)[0])

                    if time.time() - last_point_save_time >= data_val[0]:
                        latest_data_key_in_redis = 'StatusData_%s_%s_latest' %(self.client_id,self.service_name)
                        print "caluting data for key:\033[31;1m%s\033[0m" %data_key_in_redis
                        data_set = self.get_data_slice(latest_data_key_in_redis,data_val[0])
                        print "length data set :" ,len(data_set)
                        if len(data_set) >0:
                            optimized_data = self.get_optimized_data(data_key_in_redis,data_set)    #计算出优化的结果
                            if optimized_data:
                                self.save_optimized_data(data_key_in_redis,optimized_data)

                #确保在redis DB中的数据不超过setting中的值
                if self.redis_obj.llen(data_key_in_redis) >=data_val[1]:
                    self.redis_obj.lpop(data_key_in_redis)    #删除昨早的数据
        else:
            print "\033[31;1m the invalid report data:\033[0m",self.data
            raise ValueError
    ...
    def get_data_slice(self,lastest_data_key,optimization_interval):
        ...
        :return:
        '''
        all_real_data = self.redis_obj.lrange(lastest_data_key,1,-1)
