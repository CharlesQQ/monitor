...
class ClientHandler(object):
    ...
    def fetch_configs(self):
        try:
            host_obj = models.Host.objects.get(id=self.client_id)    #找出主机关联的模板
            template_list = list(host_obj.templates.select_related())

            for host_group in host_obj.host_groups.select_related():    #找出与主机关联的主机组关联的模板
            ...
            print "请求主机的配置出错，主机id %s不存在" %self.client_id

        return self.client_configs

def get_host_trigger(host_obj):
    triggers = []
    ...
    for host_group in host_obj.host_groups.select_related():
        for template in host_group.templates.select_related():
            triggers.extend(template.triggers.select_related())
    return set(triggers)
